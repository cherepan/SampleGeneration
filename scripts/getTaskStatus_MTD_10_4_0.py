...
def getExitCode(fname):
    ...
    code = -888
    for line in f:
        if not '... cmsRun - step2 - finished with status' in line:
            continue
        # code = int(re.search('... cmsRun finished with status (\d+)', line).group(1))
        code = int(re.search('... cmsRun - step2 - finished with status (\d+)', line).group(1)) ## checking only the last of the 3 steps
    return code
